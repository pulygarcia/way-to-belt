const docDefinition = (fighter) => {
    //Fights history
    const allFights = [
        ...fighter.fightsAsA.map(fight => {
            const isWinner = fight.result === `${fighter.firstName} ${fighter.lastName}`;
            return {
                opponent: `${fight.fighterB.firstName} ${fight.fighterB.lastName}`,
                result: isWinner ? 'VICTORIA' : 'DERROTA',
                resultColor: isWinner ? '#388E3C' : '#D84315',
                method: fight.method,
                rounds: fight.rounds,
                event: fight.event.name,
                date: new Date(fight.date),
                fightersString: `${fight.fighterA.lastName} vs ${fight.fighterB.lastName}`
            };
        }),
        ...fighter.fightsAsB.map(fight => {
            const isWinner = fight.result === `${fighter.firstName} ${fighter.lastName}`;
            return {
                opponent: `${fight.fighterA.firstName} ${fight.fighterA.lastName}`,
                result: isWinner ? 'VICTORIA' : 'DERROTA',
                resultColor: isWinner ? '#388E3C' : '#D84315',
                method: fight.method,
                rounds: fight.rounds,
                event: fight.event.name,
                date: new Date(fight.date),
                fightersString: `${fight.fighterA.lastName} vs ${fight.fighterB.lastName} (${fight.event.name})`
            };
        })
    ].sort((a, b) => b.date - a.date); //sort by date, last one go first

    //Fights history Table body
    const fightHistoryTableBody = [
        [
            { text: 'PELEA', style: 'tableHeader', alignment: 'left' },
            { text: 'EVENTO', style: 'tableHeader', alignment: 'left' },
            { text: 'RESULTADO', style: 'tableHeader', alignment: 'center' },
            { text: 'MÉTODO', style: 'tableHeader', alignment: 'center' },
            { text: 'RND', style: 'tableHeader', alignment: 'center' }
        ],
        ...allFights.map(fight => [
            { text: fight.fightersString, style: 'tableCell' },
            { text: fight.event, style: 'tableCell' },
            { text: fight.result, style: 'tableResult', color: fight.resultColor, alignment: 'center' },
            { text: fight.method, style: 'tableCell', alignment: 'center' },
            { text: fight.rounds.toString(), style: 'tableCell', alignment: 'center' }
        ])
    ];

    //Bonuses table
    const bonusTableBody = [
        [{ text: 'TIPO DE BONO', style: 'tableHeader', alignment: 'left' }, { text: 'MONTO', style: 'tableHeader', alignment: 'right' }],
        ...fighter.bonuses.map(bonus => [
            { text: bonus.type, style: 'tableCell' },
            { text: `$${new Intl.NumberFormat('es-ES').format(bonus.amount)}`, style: 'tableCellValue' }
        ])
    ];
    ...
    return {
        ...
        content: [
            ...
            },
            {
                columns: [
                    ...
                    //(Victories, Losses, Total) table details
                    {
                        width: '50%',
                        layout: {
                            hLineWidth: () => 0,
            ...
            },
            {
                ...
                table: {
                    headerRows: 1,
                    widths: ['45%', '25%', '15%', '8%', '7%'],
                    body: allFights.length > 0 ? fightHistoryTableBody : [
                        [
                            { text: 'PELEA', style: 'tableHeader', alignment: 'left' },
                            { text: 'EVENTO', style: 'tableHeader', alignment: 'left' },
                            { text: 'RESULTADO', style: 'tableHeader', alignment: 'center' },
                        ...
                        [{ text: 'Sin historial de peleas registrado.', colSpan: 5, alignment: 'center', color: '#666', margin: [0, 5] }, {}, {}, {}, {}]
                    ]
                }
            },
            ...
            },
            {
                ...
                table: {
                    headerRows: 1,
                    widths: ['*', 'auto'],
                    body: fighter.bonuses.length > 0 ? bonusTableBody : [
                        [{ text: 'TIPO DE BONO', style: 'tableHeader', alignment: 'left' }, { text: 'MONTO', style: 'tableHeader', alignment: 'right' }],
                        [{ text: 'Sin bonos registrados.', colSpan: 2, alignment: 'center', color: '#666', margin: [0, 5] }, {}]
                    ]
                }
            }
        ],
        ...
        footer: () => ({
            columns: [
                { text: `Reporte de Base de Datos WTB - Generado: ${new Date().toLocaleDateString('es-ES')}`, alignment: 'left', fontSize: 9, color: '#AAAAAA' },
            ],
            margin: [50, 0, 50, 0]
        }),
    };
